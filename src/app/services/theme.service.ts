...
export class ThemeService {
  ...
  private generateThemeFromColor(color: any): ThemeColors {
    ...
    const primaryRgb = this.hslToRgb(adjustedHsl[0], adjustedHsl[1], adjustedHsl[2]);
    ...
    // Gradiente
    const gradientEndRgb = this.adjustColor(primaryRgb, -0.2);
  ...
  private applyTheme(colors: ThemeColors): void {
    ...
    const gradient = `radial-gradient(circle at top right, ${colors.gradientStart}, ${colors.gradientEnd} 20%, var(--bg-primary) 50%)`;
  ...
  private safeSetStyle(property: string, value: string, element: HTMLElement = document.documentElement): void {
    if (!isPlatformBrowser(this.platformId)) return;
    
    try {
      const current = getComputedStyle(element).getPropertyValue(property).trim();
      if (current !== value) {
